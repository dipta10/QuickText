...
The current UI is compact and includes a record button, shortcut configuration, and Soniox API key controls. The frontend still toggles a local `idle`/`recording` state without creating a backend recording session.

Soniox integration adds microphone capture, credential access, WebSocket streaming, provider finalization, and network error handling. If these concerns are wired directly into the UI state, the app will duplicate trigger logic between the button, global shortcut, and future tray/menu surfaces.

The existing docs require the Rust backend to own microphone capture, provider streaming, shortcut handling, clipboard integration, secret access, and the recording/transcription source of truth.
...
4. Add secure Soniox credential reads to the start path.
5. Add microphone capture and normalized audio chunks.
6. Add the Soniox real-time WebSocket provider behind the provider interface.
7. Add final transcript and error events for the UI.
