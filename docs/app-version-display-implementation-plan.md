# App Version, Build, And Commit Display Implementation Plan

## Summary

Show the installed QuickText application version, exact release build, and source commit in Settings so a user can report which distributed artifact they are running.

Place three read-only metadata rows and one copy action in the existing **App** section at the bottom of Settings:
...
- Identify the exact source commit used to create the installed application.
- Make missing metadata non-fatal without silently misidentifying a distributed build.

## Non-Goals

- Displaying the operating system, architecture, package-manager revision, or diagnostics identifiers.
...
Show three separately labeled values:
...
- **Commit**: The shortened source revision used to build the artifact, for example `297f584`.
...
For a local build, the Build and Commit lines contain `Development`. If any lookup value is unavailable, copy the visible `Unavailable` value rather than disabling the action.
...
- **Source revision** identifies the exact commit used to build the application. The backend retains the full commit hash while Settings displays its first seven characters.
...
Return all three values from one backend query so the frontend does not need to know how the values are sourced.
...
Embed the full source revision at Rust compile time through `QUICKTEXT_SOURCE_REVISION`.

- Distributed builds receive the exact `GITHUB_SHA` for the workflow checkout.
- Settings displays the first seven characters while preserving the full value in the backend response.
- Local and manual builds without the environment value display `Development`.
- The value describes the commit used for the installed build, not the newest commit currently available on GitHub.
...
2. Set `QUICKTEXT_SOURCE_REVISION` to the workflow's exact `GITHUB_SHA` value.
3. Validate that both identifiers are non-empty and exactly match their workflow sources.
4. Pass both values to every Tauri matrix build and the separate Arch package build.
5. Fail the affected release job before publication if validation or propagation fails.
...
| Artifact | Application Version | Build ID | Source Revision |
|---|---:|---|---|
| Windows installer. | `0.1.0`. | `v0.1.0-pre.42`. | Release commit. |
| macOS disk image. | `0.1.0`. | `v0.1.0-pre.42`. | Release commit. |
| Debian, RPM, and AppImage packages. | `0.1.0`. | `v0.1.0-pre.42`. | Release commit. |
| Arch package with `pkgrel=42`. | `0.1.0`. | `v0.1.0-pre.42`. | Release commit. |

## Proposed Implementation Boundary

### Rust Backend

- Define a serializable `BuildInfo` response with `version`, `build_id`, and `source_revision` fields and serialize it with camel-case field names.
...
- Resolve Source Revision through the same metadata boundary, with the same local-development fallback.
...
- Add semantic label/value markup for Version, Build, and Commit to the existing **App** section.
...
- Populate all three metadata rows from the single backend response and shorten the displayed commit to seven characters.
...
- Style Version, Build, and Commit as compact metadata rows consistent with existing Settings typography.
...
- Set Source Revision from `GITHUB_SHA` for every platform matrix build.
- Validate both values before artifact publication.
- Pass the same values into the Arch container and export them during `makepkg`'s application build.
...
- If the query fails, display `Unavailable` for all values and keep QuickText fully usable.
...
- Keep all values as selectable text.
...
- Verify a supplied source revision is returned unchanged.
...
- Verify Version, Build, and Commit render from one successful response.
...
- Verify a release build displays and copies the shortened source revision.
...
- Verify every release job receives a non-empty `QUICKTEXT_SOURCE_REVISION` equal to `GITHUB_SHA`.
...
3. Confirm Version matches the packaged development version and Build and Commit show `Development`.
4. Copy build information and confirm the clipboard contains the exact four-line development payload.
...
- The bottom **App** section in Settings shows separately labeled Version, Build, and Commit values.
...
- Every artifact reports the source commit used to build it.
...
- One **Copy build information** action copies the exact visible values in the agreed four-line format.
...
- Version, Build, and Commit are not persisted as user settings and are not added to recording state.
- Platform metadata, update behavior, and diagnostics remain out of scope.
- The layout remains accessible and usable at the minimum supported window size.

## Implementation Sequence

1. Add compile-time Build ID and Source Revision handling and the backend `BuildInfo` query.
2. Add backend contract tests for release, development, and serialization behavior.
3. Propagate and validate `RELEASE_TAG` and `GITHUB_SHA` in every CD packaging path.
...
- **One combined string or separate rows?** Separate rows. Application Version, Build ID, and Source Revision answer different questions.
...
- **Interaction?** One action copies all values in a support-ready block.
- **Development identity?** `Development`, because a commit-derived value can misrepresent a dirty local build.
- **Metadata boundary?** One backend query returns all values so sourcing remains outside the UI.
...
- **Source revision?** Show the shortened build commit while embedding the full value supplied by CD.
